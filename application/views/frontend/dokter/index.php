										...
										<?php foreach ($d->jadwal as $jadwal): ?>
											<div class="schedule-item mb-2 d-flex justify-content-between">
												<small class="text-primary mb-0">
													<i class="fas fa-calendar-alt me-1"></i>
													<?php
													$hari = $jadwal['hari_list'];

													// Urutan hari dalam seminggu
													$urutan_hari = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu", "Minggu"];
													$index_hari = array_flip($urutan_hari);

													// Ubah jadi index
													$hari_index = array_map(function ($h) use ($index_hari) {
														return $index_hari[$h];
													}, $hari);

													sort($hari_index);

													// Cek apakah semua hari berurutan
													$berurutan = true;
													for ($i = 1; $i < count($hari_index); $i++) {
														if ($hari_index[$i] !== $hari_index[$i - 1] + 1) {
															$berurutan = false;
															break;
														}
													}
